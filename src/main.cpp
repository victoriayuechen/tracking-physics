...
int main() {
    ...
    tracker.setUpTracking("../movement/frame_0.pcd", particleCount, variance, delta, epsilon, binSize);
    ...
    while (tracker.frameCount < maxFrames) {
        if (pcl::io::loadPCDFile<RefPointType> ("../movement/frame_" + std::to_string(0) + ".pcd", *cloud) == -1) {
