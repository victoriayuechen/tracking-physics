...
// Path for loading series of point cloud data 
...
std::string getNextFileName() {
    return path + std::to_string(frame_count) + ".pcd"; 
}
...
bool drawParticles (pcl::visualization::PCLVisualizer& viz)
{
    ...
    // Only draw if particles and new cloud exist 
...
// Tracking of target using point cloud at each frame 
void trackFrame(const CloudConstPtr &cloud) 
{
    std::lock_guard<std::mutex> lock (mtx_);
    cloud_pass_.reset (new Cloud);
    cloud_pass_downsampled_.reset (new Cloud);
    ...
    // Ignore point cloud until we have 10 frames, might remove. 
    if (frame_count < 10) {
        frame_count++; 
    } else {
  	    // Track the object
...
// Load the point cloud at specified frame 
CloudConstPtr get_cloud_at_frame(long frame) 
{
    CloudConstPtr cloud; 
    
    if (pcl::io::loadPCDFile<RefPointType>(getNextFileName(), *cloud) == -1) {
        PCL_ERROR("PCD file loading failed! \n"); 
        return null; 
    }
